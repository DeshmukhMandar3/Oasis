...
let url='http://localhost:8080/adminregister'
...
//securety code
const [securetycode,setsecuretycode]=useState(false)
const [code,setcode] =useState('')
const chengethecode=()=>{
if(code==="osasa"){
  setsecuretycode(!securetycode)
}else{
  toast({
...
const {adminauth,setadminauth,userdata,adminsetuserdata}=useContext(Authcontext)
...
const getregisterdata=async(obj)=>{
      setlading(false)
  try {
      let resdata=await axios.get("http://localhost:8080/adminregister") 
...
const postregisterdata=async(obj)=>{
      try {
            let registerdatas=await axios.post("http://localhost:8080/adminregister",obj)
            toast({
...
const postuserdatadata=async(obj)=>{
      try {
            let userdatas=await axios.post("http://localhost:8080/adminuser",obj)
...
if(!adminauth){
      return( 
              <>
      <Button ml='1000px' onClick={onOpen}>AdminLogin</Button>
...
>
 ...
 <Text ml='300px' fontWeight={'bold'} fontSize={'20px'}>Admin</Text>
  <TabList ml='20px'>
...
/>} </Box> : <Box ml='20px'>
  <Text mt='20px' color={'red'} fontWeight='bold' fontSize={'20px'}>Security Code*</Text>
  <Input mt='20px' variant='flushed' w='70%' type={'text'} value={code} placeholder='Enter Security Code' onChange={e => setcode(e.target.value)}   borderBottom="1px solid black" />
  <br></br>
  <Button mt='50px' width={'300px'} onClick={chengethecode}>submit</Button>
  </Box> }
  </GridItem>: 
